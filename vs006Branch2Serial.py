...
def algoMain(graph):
    ...
    roots = ed.unsatNodes()  ##finds unsaturated nodes
    
    
    ed.findAugmentingPaths()
...
class EdmondsMethods:
    ...
    def unsatNodes(self):
        ...
        for match in self.graph1.matchings:
            self.sat.add(match.node1)
            self.sat.add(match.node2)
        for node in self.graph1.nodes:
            if node not in self.sat:
                self.unsat.add(node)

        self.unsat=sorted(self.unsat, key=lambda node: node.node_id)        
        return list(self.unsat)


    def findAugmentingPaths(self):
        for unsat_node in self.unsatNodes():
            print(unsat_node.node_id)
            # Initialize search from unsat_node
            path = [unsat_node]  # Start with the current unsaturated node
            visited = set()  # Track visited nodes to detect cycles
            self.searchForAugmentingPath(unsat_node, path, visited)

    def searchForAugmentingPath(self, current_node, path, visited, prev_node=None):
        if current_node in visited:
            # Cycle detected, potential blossom
            self.checkForBlossom(path, current_node)
            return False
        visited.add(current_node)
        
        for neighbor in self.graph1.unMatchedNeigh(current_node):
            if neighbor == prev_node:
                continue  # Avoid trivial cycle
            
            path.append(neighbor)  # Tentatively add neighbor to path
            
            # Check if we've reached another unsaturated node
            if neighbor in self.unsat and neighbor != path[0]:
                # Found an augmenting path!
                return True
            
            # Recursively search from the neighbor
            if self.searchForAugmentingPath(neighbor, path, visited, current_node):
                return True
            
            path.pop()  # Backtrack
        
        return False  # No augmenting path found from this node

    def checkForBlossom(self, path, cycle_start):
        cycle_index = path.index(cycle_start)
        cycle = path[cycle_index:]
        if len(cycle) % 2 == 1:
            print("Blossom detected:", [node.node_id for node in cycle])
            # The actual shrinking of the blossom is complex and requires significant additional logic
            # You'll need to contract the cycle into a single node and adjust the graph accordingly
            # This might involve creating a new graph or modifying the current graph's structure

    def shrinkBlossom(self, blossom):
        # Placeholder for logic to shrink a blossom
        pass
    
    def expandBlossom(self, blossom):
        # Placeholder for logic to expand a blossom back to its original nodes
        pass
