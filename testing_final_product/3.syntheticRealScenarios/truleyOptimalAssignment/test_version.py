...
class Invigilator:
    def __init__(self, id, name, avail, lead, size_pref):
        self.id = int(id)
        self.name = name
        # Handle the avail if it's empty or not a string
...
def create_decision_variables(invigilators, exam_sessions):
    ...
    y = pulp.LpVariable.dicts("unavailable",
                              [(invigilator.name, time_slot)
                               for invigilator in invigilators
                               for time_slot in exam_sessions
                               if invigilator.avail and time_slot not in invigilator.avail],  # Only penalize if availability list is not empty
                              cat='Binary')
...
def export_results_to_excel(invigilators, exam_sessions, results, exam_colours, unmet_invig_requirements, unmet_lead_requirements, unavailable_assignments, unassigned_invigilators, objective_value, status):
    max_time_slot = max(exam_sessions.keys())
    wb = Workbook()
    ws = wb.active
    ws.title = "Invigilator Assignments"

    # Add solution status at the top of the sheet
    ws["A1"] = "Solution Status:"
    ws["B1"] = status  # Optimal or infeasible
    ...
    for invigilator_name in unassigned_invigilators:
        ws[f"{start_col_letter}{row_offset}"] = invigilator_name
        row_offset += 1
...
def extract_exam_ids_from_cell(cell_value):
    if isinstance(cell_value, str):
        ...
        # If cell_value is not a string, return an empty list (or handle as needed)
        return []
...
def print_unassigned_invigilators(invigilators, results, max_time_slot):
    ...
    return unassigned_invigilators

# Main Execution
def main():
    invig_file = get_resource_path('75_invigilators.xlsx')
    ...
    unassigned_invigilators = print_unassigned_invigilators(invigilators, results, max_time_slot)
